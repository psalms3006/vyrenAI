...
class VoiceRuntime:
    ...
    def _start_gemini_live(self, api_key: str):
        ...
        # Get tool declarations in Gemini format (raw dicts — NOVA pattern)
        gemini_tools = self._ctx.get("gemini_tools", [])
        ...
        logger.info("Voice: Gemini Live engine started (shared architecture v2.2)")
    ...
    def _build_live_config(self) -> types.LiveConnectConfig:
        ...
        This means if memory changes during a session, the NEXT reconnect
        will pick up those changes automatically. No stale context.
        """
        # Rebuild system prompt with fresh memory/time context
        system_prompt = self._build_system_prompt()

        # Get tools (may have been updated by plugin loading, etc.)
        gemini_tools = self._ctx.get("gemini_tools", [])

        logger.info(
            "Building LiveConnectConfig: prompt=%d chars, tools=%d items",
            len(system_prompt),
            len(gemini_tools),
        )

        # Match NOVA's exact config structure
    ...
    def _on_turn_complete(self, turn: TurnTranscription):
        """Handle completed voice turn — audit, memory."""
        audit = self._ctx.get("audit")
        if audit:
            if turn.user_text:
    ...
    def _start_fallback(self):
        """Start fallback voice mode with auto-recovery to Gemini Live."""
        self._active = True
        self._fallback_mode = True
        self._fallback_reason = "no_api_key"
        logger.info("Voice: Fallback mode starting (local STT/TTS)")
    ...
    def _fallback_main(self):
        """Offline voice loop with automatic Gemini Live recovery."""
        import time

        logger.info("Fallback voice: active. Will auto-recover to Gemini Live.")
